...
public class MagacinDialog extends JDialog{
	private MagacinDialog ovaj;
	...
	public MagacinDialog(Magacin magacin) {
		ovaj = this;
		izmena = true;

		initialize();

		tfNaziv.setText(magacin.getNaziv());
		cbOrganizacionaJedinica.setSelectedItem(magacin.getOrganizacionaJedinica());
	...
	private void initialize() {
				...
				
		{
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
						...
						if (proveriValidnostPolja()) {
							...
							if (izmena) {
								rezultat = Utilities.getDataAccessFactory()
										.getMagacinDataAccess().azurirajMagacin(magacin);
								if (!rezultat)
									JOptionPane.showMessageDialog(ovaj,
											"Magacin nije uspešno ažuriran!",
											"Poruka",
											JOptionPane.INFORMATION_MESSAGE);
							} else {
								rezultat = Utilities.getDataAccessFactory()
										.getMagacinDataAccess().dodajMagacin(magacin);
								if (!rezultat)
									JOptionPane.showMessageDialog(ovaj,
											"Magacin nije uspešno dodan!",
											"Poruka",
											JOptionPane.INFORMATION_MESSAGE);
							}
							if (rezultat) {
				...
				okButton.setIcon(new ImageIcon(MagacinDialog.class
