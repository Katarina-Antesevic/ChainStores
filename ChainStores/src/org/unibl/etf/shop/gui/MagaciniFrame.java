...
public class MagaciniFrame extends JFrame{
	private MagaciniFrame ovaj;
	private List<Magacin> magacin;

	private Magacin odabraniMagacin;
	 ...
	 * Create the frame.
	 */
	public MagaciniFrame(boolean odabirMagacina) {
		ovaj = this;
		magacin = Utilities.getDataAccessFactory().getMagacinDataAccess()
				.magaciniSvi();
	...
	public Magacin getOdabraniMagacin() {
		return odabraniMagacin;
	}

	private void osveziTabelu() {
		if (Utilities.isSearchPatternValid(tfNaziv.getText())) {
			magacin = Utilities.getDataAccessFactory().getMagacinDataAccess()
					.magacini(tfNaziv.getText());

			MagacinTableModel ftm = (MagacinTableModel) table.getModel();
			ftm.setPodaci(magacin);
		...
		if (btnDodati == null) {
			...
			btnDodati.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					MagacinDialog pd = new MagacinDialog();
					pd.setVisible(true);
					if (pd.getDialogResult().equalsIgnoreCase("OK")) {
						osveziTabeluZaSve();
						JOptionPane.showMessageDialog(ovaj,
								"Novi magacin je uspešno dodan!", "Poruka",
								JOptionPane.INFORMATION_MESSAGE);
					}
				}
			});
			btnDodati.setIcon(new ImageIcon(MagaciniFrame.class
					.getResource(Utilities.IMAGE_RESOURCES_PATH + "Add_32.png")));
			btnDodati.setToolTipText("Dodati novi magacin");
			...
			btnIzmeniti.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (table.getSelectedRow() == -1) {
						...
						Magacin odabraniMagacin = ((MagacinTableModel) table
								.getModel()).getMagacinAtRow(table
								.getSelectedRow());
						MagacinDialog pd = new MagacinDialog(odabraniMagacin);
						pd.setVisible(true);
						if (pd.getDialogResult().equalsIgnoreCase("OK")) {
							osveziTabeluZaSve();
							JOptionPane.showMessageDialog(ovaj,
									"Magacin je uspešno ažuriran!", "Poruka",
									JOptionPane.INFORMATION_MESSAGE);
						}
					}
				}
			});
			btnIzmeniti.setIcon(new ImageIcon(MagaciniFrame.class
					.getResource(Utilities.IMAGE_RESOURCES_PATH + "Edit_32.png")));
			btnIzmeniti.setToolTipText("Izmeniti odabrani magacin");
	...
	private JButton getBtnObrisati() {
		if (btnObrisati == null) {
				...
				public void actionPerformed(ActionEvent e) {
					if (table.getSelectedRow() == -1) {
						...
						int odabir = JOptionPane
								.showOptionDialog(
										ovaj,
										"Da li ste sigurni da želite obrisati odabrani magacin?",
										"Potvrda brisanja",
						...
						if (odabir == JOptionPane.YES_OPTION) {
							if (Utilities
									.getDataAccessFactory().getMagacinDataAccess()
									.obrisiMagacin(
											odabraniMagacin.getNaziv())) {
								osveziTabeluZaSve();
								JOptionPane.showMessageDialog(ovaj,
										"Magacin je uspešno obrisan!",
										"Poruka",
										JOptionPane.INFORMATION_MESSAGE);
							} else
								JOptionPane.showMessageDialog(ovaj,
										"Magacin nije uspešno obrisan!",
										"Poruka",
										JOptionPane.INFORMATION_MESSAGE);
						}
					}
			...
			btnObrisati.setIcon(new ImageIcon(MagaciniFrame.class
					.getResource(Utilities.IMAGE_RESOURCES_PATH + "Delete_32.png")));
			btnObrisati.setToolTipText("Obrisati odabrani magacin");
	...
	private JButton getBtnPrihvatiti() {
		if (btnPrihvatiti == null) {
			...
			btnPrihvatiti.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (table.getSelectedRow() == -1) {
						...
						odabraniMagacin = ((MagacinTableModel) table.getModel())
								.getMagacinAtRow(table.getSelectedRow());
						ovaj.getToolkit()
								.getSystemEventQueue()
								.postEvent(
			...
			btnPrihvatiti.setIcon(new ImageIcon(MagaciniFrame.class
					.getResource(Utilities.IMAGE_RESOURCES_PATH + "Check_32.png")));
			btnPrihvatiti.setToolTipText("Prihvatiti odabrani magacin");
			...
			btnPretraziti.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (Utilities.isSearchPatternValid(tfNaziv
						...
						JOptionPane.showMessageDialog(ovaj,
								"Naziv magacina nije pravilno popunjen!",
	...
	private JTable getTable() {
		if (table == null) {
			table = new JTable(new MagacinTableModel(magacin));
