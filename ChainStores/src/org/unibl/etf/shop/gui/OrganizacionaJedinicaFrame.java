...
public class OrganizacionaJedinicaFrame extends JFrame {
	private OrganizacionaJedinicaFrame ovaj;
	private List<OrganizacionaJedinica> oj;

	private OrganizacionaJedinica odabranaOJ;
	 ...
	 * Create the frame.
	 */
	public OrganizacionaJedinicaFrame(boolean odabirOJ) {
		ovaj = this;
		oj = Utilities.getDataAccessFactory().getOrganizacionaJedinicaDataAccess()
				.organizacioneJediniceSve();
	...
	public OrganizacionaJedinica getOdabranaOJ() {
		return odabranaOJ;
	}
	...
	private void osveziTabelu() {
			...
			oj = Utilities.getDataAccessFactory().getOrganizacionaJedinicaDataAccess()
					.organizacioneJedinice(Integer.valueOf(tfid.getText()));

			OrganizacionaJedinicaTableModel ftm = (OrganizacionaJedinicaTableModel) table.getModel();
			ftm.setPodaci(oj);
	...
	//ovako izgleda onaj dio za pretrazivanje
	private JPanel getPanelPretraga() {
		if (panelPretraga == null) {
			...
			panelPretraga.add(getLblid());
			panelPretraga.add(getTfid());
		...
		if (btnDodati == null) {
			...
			btnDodati.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					OrganizacionaJedinicaDialog pd = new OrganizacionaJedinicaDialog();
					pd.setVisible(true);
					if (pd.getDialogResult().equalsIgnoreCase("OK")) {
						osveziTabeluZaSve();
						JOptionPane.showMessageDialog(ovaj,
								"Nova organizaciona jedinina je uspešno dodana!", "Poruka",
								JOptionPane.INFORMATION_MESSAGE);
					}
				}
			});
			btnDodati.setIcon(new ImageIcon(OrganizacionaJedinicaFrame.class
					.getResource(Utilities.IMAGE_RESOURCES_PATH + "Add_32.png")));
			btnDodati.setToolTipText("Dodati novou organizacionu jedinicu");
			...
			btnIzmeniti.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (table.getSelectedRow() == -1) {
						...
						OrganizacionaJedinica odabranaOJ = ((OrganizacionaJedinicaTableModel) table
								.getModel()).getOrganizacionaJedinicaAtRow(table
								.getSelectedRow());
						OrganizacionaJedinicaDialog pd = new OrganizacionaJedinicaDialog(odabranaOJ);
						pd.setVisible(true);
						if (pd.getDialogResult().equalsIgnoreCase("OK")) {
							osveziTabeluZaSve();
							JOptionPane.showMessageDialog(ovaj,
									"Organizaciona jedinica je uspešno ažurirana!", "Poruka",
									JOptionPane.INFORMATION_MESSAGE);
						}
					}
				}
			});
			btnIzmeniti.setIcon(new ImageIcon(OrganizacionaJedinicaFrame.class
					.getResource(Utilities.IMAGE_RESOURCES_PATH + "Edit_32.png")));
			btnIzmeniti.setToolTipText("Izmeniti odabranu organizacionu jedinicu");
	...
	private JButton getBtnObrisati() {
		if (btnObrisati == null) {
			...
			btnObrisati.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (table.getSelectedRow() == -1) {
						...
						OrganizacionaJedinica odbranaOJ = ((OrganizacionaJedinicaTableModel) table
								.getModel()).getOrganizacionaJedinicaAtRow(table
								.getSelectedRow());
						int odabir = JOptionPane
								.showOptionDialog(
										ovaj,
										"Da li ste sigurni da želite obrisati odabranu organizacionu jedinicu?",
										"Potvrda brisanja",
						...
						if (odabir == JOptionPane.YES_OPTION) {
							if (Utilities
									...
									.getOrganizacionaJedinicaDataAccess()
									.obrisiOrganizacionuJedinicu(
											odbranaOJ.getId_OrganizacioneJedinice())) {
								osveziTabeluZaSve();
								JOptionPane.showMessageDialog(ovaj,
										"Organizaciona jedinica je uspešno obrisana!",
										"Poruka",
										JOptionPane.INFORMATION_MESSAGE);
							} else
								JOptionPane.showMessageDialog(ovaj,
										"Organizaciona jedinica nije uspešno obrisana!",
										"Poruka",
										JOptionPane.INFORMATION_MESSAGE);
						}
					}
			...
			btnObrisati.setIcon(new ImageIcon(OrganizacionaJedinicaFrame.class
					.getResource(Utilities.IMAGE_RESOURCES_PATH + "Delete_32.png")));
			btnObrisati.setToolTipText("Obrisati odabranu organizacionu jedinicu");
	...
	private JButton getBtnPrihvatiti() {
		if (btnPrihvatiti == null) {
			...
			btnPrihvatiti.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (table.getSelectedRow() == -1) {
						...
						odabranaOJ = ((OrganizacionaJedinicaTableModel) table.getModel())
								.getOrganizacionaJedinicaAtRow(table.getSelectedRow());
						ovaj.getToolkit()
								.getSystemEventQueue()
								.postEvent(
			...
			btnPrihvatiti.setIcon(new ImageIcon(OrganizacionaJedinicaFrame.class
					.getResource(Utilities.IMAGE_RESOURCES_PATH + "Check_32.png")));
			btnPrihvatiti.setToolTipText("Prihvatiti odabranu organizacionu jedinicu");
	...
	*/
	...
	private JButton getBtnPretraziti() {
		if (btnPretraziti == null) {
			...
			btnPretraziti.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (Utilities.isSearchPatternValid(tfid
							.getText()))
						osveziTabelu();
					else
						JOptionPane.showMessageDialog(ovaj,
								"ID organizacione jedinice nije pravilno popunjen!",
	...
	private JTable getTable() {
		if (table == null) {
			table = new JTable(new OrganizacionaJedinicaTableModel(oj));
