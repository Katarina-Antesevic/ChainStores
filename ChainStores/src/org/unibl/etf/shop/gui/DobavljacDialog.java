...
public class DobavljacDialog extends JDialog  {
	private DobavljacDialog ovaj;
	...
	public DobavljacDialog(Dobavljac dobavljac) {
		ovaj = this;
		izmena = true;

		initialize();

		tfJIB.setText(dobavljac.getJib());
		tfJIB.setEditable(false);
		tfNaziv.setText(dobavljac.getNaziv());
		tfNaziv.setEditable(false);
		tfEmail.setText(dobavljac.getEmail());
		tfTelefon.setText(dobavljac.getTelefon());
		tfAdresa.setText(dobavljac.getAdresa());
		cbBrojPoste.setSelectedItem(dobavljac.getMJESTO_BrojPoste());
	...
	private void initialize() {
		...
		
		{
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						if (proveriValidnostPolja()) {
							Dobavljac dobavljac = new Dobavljac(tfJIB.getText(),
							...
							if (izmena) {
								rezultat = Utilities.getDataAccessFactory()
										.getDobavljacDataAccess().azurirajDobavljaca(dobavljac);
								if (!rezultat)
									JOptionPane.showMessageDialog(ovaj,
											"Dobavljac nije uspešno ažuriran!",
											"Poruka",
											JOptionPane.INFORMATION_MESSAGE);
							} else {
								rezultat = Utilities.getDataAccessFactory()
										.getDobavljacDataAccess().dodajDobavljaca(dobavljac);
								if (!rezultat)
									JOptionPane.showMessageDialog(ovaj,
											"Dobavljac nije uspešno dodan!",
											"Poruka",
											JOptionPane.INFORMATION_MESSAGE);
							}
							if (rezultat) {
				...
				okButton.setIcon(new ImageIcon(DobavljacDialog.class
